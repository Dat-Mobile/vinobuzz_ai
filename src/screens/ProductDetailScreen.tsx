...
export const ProductDetailScreen = ({ route, navigation, offline }: ProductDetailContentProps) => {
  ...
  const product = useMemo<Product>(() => productById(route.params.productId), [route.params.productId]);
        ...
        <>
          <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.body}>
              {route.params.fromChat ? <Text style={styles.chatHint}>Opened from chat recommendation</Text> : null}
              ...
              <TouchableOpacity onPress={() => setExpanded((current) => !current)} style={styles.expandHeader}>
          ...
          <LinearGradient colors={['rgba(246,241,235,0)', 'rgba(246,241,235,0.95)']} style={styles.ctaShadow} />

          <View style={[styles.stickyCta, { paddingBottom: 12 + insets.bottom }]}>
            ...
            <TouchableOpacity disabled={offline} style={[styles.addButton, offline && styles.addButtonDisabled]}>
              <Text style={styles.addButtonText}>{offline ? 'Unavailable Offline' : 'Add To Cart'}</Text>
            </TouchableOpacity>
          </View>
        </>
      )}
